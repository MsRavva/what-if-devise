...
## Текущий статус: ✅ v1.1.0 (Refinement Update)

**Дата обновления:** Февраль 2026

### Основные нововведения v1.1.0:
- **Летописец**: Полная стандартизация терминологии роли пользователя.
- **Облачные сохранения**: Интеграция с Supabase для сохранения игрового прогресса в Хорроре и Приключении.
- **ИИ-действия**: Динамическая обработка любых пользовательских команд через нейросеть (Fallback handler).
- **Dark Mode 2.0**: Настоящий «ночной режим» с сохранением книжной эстетики.
- **UI Polishing**: Исправлены проблемы со смещением интерфейса на игровых страницах.

## Архитектура системы

```mermaid
graph TD
    subgraph "Клиент (Next.js 14)"
        A["/ - Главная"] --> B["/what-if - Создание"]
        A --> C["/cinema - Режим Кино"]
        A --> D["/templates - Шаблоны"]
        A --> E["/auth - Авторизация"]
        B --> F{API /generate-scenario}
        C --> F
        D --> G[Локальные данные]
        B --> H[Supabase - Опционально]
        C --> H
    end
...
### Этап 8: Улучшение игровых механик (Refinement) ✅
- [x] Стандартизация роли «Летописец»
- [x] Реализация облачных сохранений (Supabase game_saves)
- [x] Интеграция ИИ-обработки свободных команд в играх
- [x] Исправление верстки игровых страниц (layout stability)
- [x] Обновление темной темы (aesthetic parchment)
- [x] Удаление спойлеров в Хоррор-режиме
